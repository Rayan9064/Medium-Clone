...
// .wrangler/tmp/bundle-wO0vTD/checked-fetch.js
...
// .wrangler/tmp/bundle-wO0vTD/middleware-loader.entry.ts
init_checked_fetch();
init_modules_watch_stub();

// .wrangler/tmp/bundle-wO0vTD/middleware-insertion-facade.js
init_checked_fetch();
init_modules_watch_stub();
...
blogRouter.use("/*", async (c, next) => {
  ...
  const token = await verify2(jwt2, c.env.JWT_SECRET);
  if (!token) {
    c.status(401);
...
blogRouter.put("/", async (c) => {
  ...
  try {
    ...
    console.log(e);
    return c.json({ e: "Something went wrong while updating the blog" });
  }
});
blogRouter.get("/:id", (c) => {
